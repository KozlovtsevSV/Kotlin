...
enum class ВescrizioneWeather{
    ...
    SUNNY{ override fun toString() = "Солнечно"},
    RAIN{ override fun toString() = "Дождь"},
    SNOW{ override fun toString() = "Снег"}
    }

fun randomВescrizioneWeather() : ВescrizioneWeather{

    val size = ВescrizioneWeather.values().size
    val randIndex: Int = (0..size-1).random()
    return ВescrizioneWeather.values().get(randIndex)
}

fun getDefaultCity() = City("Новосибирск", 55.00835259999999,82.93573270000002)
